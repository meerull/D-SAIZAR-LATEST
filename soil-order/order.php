
<?php include('partials-front/menu.php'); ?>

<?php 
    //CHeck whether soil id is set or not
    if(isset($_GET['soil_id']))
    {
        //Get the soil id and details of the selected soil
        $soil_id = $_GET['soil_id'];

        //Get the DEtails of the SElected soil
        $sql = "SELECT * FROM tbl_soil WHERE id=$soil_id";
        //Execute the Query
        $res = mysqli_query($conn, $sql);
        //Count the rows
        $count = mysqli_num_rows($res);
        //CHeck whether the data is available or not
        if($count==1)
        {
            //WE Have DAta
            //GEt the Data from Database
            $row = mysqli_fetch_assoc($res);

            $title = $row['title'];
            $price = $row['price'];
            $image_name = $row['image_name'];
        }
        else
        {
            //soil not Availabe
            //REdirect to Home Page
            header('location:'.SITEURL);
        }
    }
    else
    {
        //Redirect to homepage
        header('location:'.SITEURL);
    }
?>

<!-- soil sEARCH Section Starts Here -->
<section class="soil-order">
    <div class="container">        
        <h2 class="text-center text-white">Please confirm to place order</h2>

        <form action="" method="POST" class="order">
            <fieldset>
                <legend style="color:white;">Selected Soil</legend>

                <div class="soil-menu-img">
                    <?php 
                    
                        //CHeck whether the image is available or not
                        if($image_name=="")
                        {
                            //Image not Availabe
                            echo "<div class='error'>Image not Available.</div>";
                        }
                        else
                        {
                            //Image is Available
                            ?>
                            <img src="<?php echo SITEURL; ?>images/soil/<?php echo $image_name; ?>" alt="Chicke Hawain Pizza" class="img-responsive img-curve">
                            <?php
                        }
                    
                    ?>
                    
                </div>

                <div class="soil-menu-desc">
                    <h3 style="color:white;"><?php echo $title; ?></h3>
                    <input type="hidden" name="soil" value="<?php echo $title; ?>">

                    <p class="soil-price" style="color:white;">RM<?php echo $price; ?></p>
                    <input type="hidden" name="price" value="<?php echo $price; ?>">

                    <div class="order-label" style="color:white;">Quantity</div>
                    <input type="number" name="qty" class="input-responsive" value="1" required>
                    
                </div>

            </fieldset>
            
            <fieldset>
                <div class="order-label" style="color:white;">Delivery Address</div>
                <textarea name="delivery_address" class="input-responsive" required></textarea>
            </fieldset>

            <fieldset>
                <input type="submit" name="submit" value="Confirm Order" class="btn btn-primary">
            </fieldset>


        </form>

        <?php 

            //CHeck whether submit button is clicked or not
            if(isset($_POST['submit']))
            {
                if(empty($_SESSION["u_id"]))
{
header('location:login.php');
}
else{
  // Get all the details from the form

  $soil = $_POST['soil'];
  $price = $_POST['price'];
  $qty = $_POST['qty'];

  $total = $price * $qty; // total = price x qty 

date_default_timezone_set('Asia/Kuala_Lumpur'); 

  $order_date = date("Y-m-d H:i:s"); //Order DAte

  $status = "Ordered";  // Ordered, On Delivery, Delivered, Cancelled
  $u_id=$_SESSION["u_id"];
  
  $delivery_address = mysqli_real_escape_string($conn, $_POST['delivery_address']);



  //Save the Order in Databaase
  //Create SQL to save the data
  $sql2 = "INSERT INTO tbl_order SET 
      soil = '$soil',
      price = $price,
      qty = $qty,
      total = $total,
      order_date = '$order_date',
      status = '$status',
      u_id='$u_id',
      delivery_address = '$delivery_address'
       ";

  //echo $sql2; die();

  //Execute the Query
  $res2 = mysqli_query($conn, $sql2);

  //Check whether query executed successfully or not
  if($res2==true)
  {
      //Query Executed and Order Saved
      
      $_SESSION['order'] = "<div class='success text-center'>soil Ordered Successfully.</div>";
      header('location:'.SITEURL);
  }
  else
  {
      //Failed to Save Order
      $_SESSION['order'] = "<div class='error text-center'>Failed to Order soil.</div>";
      header('location:'.SITEURL);
  }
}
            }
        ?>

    </div>
</section>
<!-- soil sEARCH Section Ends Here -->

<?php include('partials-front/footer.php'); ?>